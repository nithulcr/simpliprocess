...
const StartNode = (props: NodeProps<StartNodeData>) => {
  return (
    <div className="react-flow__node-custom text-center" style={{ '--node-bg-color': props.data.color || '#dcfce7', '--node-text-color': props.data.textColor || '#065f46', border: props.selected ? '1px solid #787878' : '1px solid var(--node-text-color)', padding: '10px', borderRadius: '50%', width: props.data.width || 100, height: props.data.height || 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
